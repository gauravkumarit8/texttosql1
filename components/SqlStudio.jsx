...
const App = () => {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      setResponse({
        ...
        results: [],
        errors: { messages: ["Failed to connect to n8n. Please ensure n8n is running and the workflow is in 'Test' mode."] },
  ...
  const getEngineIcon = (engine) => {
    switch(engine) {
      case 'spark': return <Zap className="w-3 h-3" />;
      case 'postgres': return <Database className="w-3 h-3" />;
      case 'mysql': return <Database className="w-3 h-3" />;
      case 'clickhouse': return <Database className="w-3 h-3" />;
  ...
  const getEngineColor = (engine) => {
    switch(engine) {
      case 'spark': return 'text-orange-400 bg-orange-500/10 border-orange-500/30';
      case 'postgres': return 'text-blue-400 bg-blue-500/10 border-blue-500/30';
      case 'mysql': return 'text-cyan-400 bg-cyan-500/10 border-cyan-500/30';
      case 'clickhouse': return 'text-yellow-400 bg-yellow-500/10 border-yellow-500/30';
  ...
  return (
    <div className="flex h-screen bg-[#0F1115] text-slate-200 font-sans selection:bg-blue-500/30 overflow-hidden">
      ...
      {/* --- Sidebar --- */}
      <aside className={`${sidebarOpen ? 'w-72' : 'w-0'} bg-[#16191F] border-r border-slate-800 transition-all duration-300 flex flex-col overflow-hidden shrink-0`}>
        <div className="p-6 flex items-center gap-3 border-b border-slate-800">
          <div className="bg-blue-600 p-2 rounded-lg">
            <Database className="w-5 h-5 text-white" />
          </div>
          <span className="font-bold text-lg tracking-tight">Text To SQL Studio</span>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 p-1 bg-slate-900/60 rounded-xl border border-slate-800">
              {['postgres', 'mysql', 'clickhouse', 'spark'].map((type) => (
                <button
                  key={type}
                  onClick={() => setDbEngine(type)}
                ...
                >
                  {type === 'spark' && <Zap className="w-3 h-3" />}
                  {type === 'postgres' ? 'PostgreSQL' : type === 'clickhouse' ? 'ClickHouse' : type}
                </button>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-3 bg-slate-900/40 p-4 rounded-xl border border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-500 uppercase">Database</label>
                    <input 
                      type="text"
                      className="w-full bg-[#0F1115] border border-slate-700 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-blue-500 transition"
                      placeholder="name"
                ...
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 uppercase">User</label>
                  <input 
                    type="text"
                    className="w-full bg-[#0F1115] border border-slate-700 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-blue-500 transition"
                    value={dbConfig.user}
                ...
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 uppercase">Password</label>
                  <div className="relative">
          ...
          {dbEngine === 'spark' && (
            <div>
              ...
              <div className="space-y-3 bg-orange-900/10 p-4 rounded-xl border border-orange-500/20">
                {/* Catalog */}
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 uppercase">Catalog (Optional)</label>
                  <input 
                    type="text"
                    ...
                    onChange={(e) => setSparkConfig({...sparkConfig, catalog: e.target.value})}
                  />
                  <p className="text-[8px] text-slate-600 italic">Leave empty if not using Unity Catalog</p>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    onChange={(e) => setSparkConfig({...sparkConfig, database: e.target.value})}
                  />
                </div>
                ...
                <div className="border-t border-slate-700/50 pt-3">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-3 h-3 transition-transform ${showSparkEndpoint ? 'rotate-180' : ''}`} />
                  </button>
                  {showSparkEndpoint && (
                    <div className="mt-2 space-y-1">
                      <input 
                        ...
                        onChange={(e) => setSparkConfig({...sparkConfig, schemaEndpoint: e.target.value})}
                      />
                      <p className="text-[8px] text-slate-600 italic">External API to fetch table schemas</p>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-slate-700/50 pt-3">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-3 h-3 transition-transform ${showSparkSchema ? 'rotate-180' : ''}`} />
                  </button>
                  {showSparkSchema && (
                    <div className="mt-2 space-y-1">
                      <textarea 
  ...
  {"table": "users", "column_name": "name", "data_type": "STRING"}
                        ...
                        onChange={(e) => setSparkConfig({...sparkConfig, schema: e.target.value})}
                      />
                      <p className="text-[8px] text-slate-600 italic">JSON array of column definitions</p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-orange-500/5 border border-orange-500/20 rounded-lg p-2 mt-2">
                  <p className="text-[9px] text-orange-400/80 leading-relaxed">
                    ⚡ Spark SQL will be generated but not executed. Use the generated query in your Spark cluster.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {response?.metadata?.found_in_db?.map((table, i) => (
                <div key={i} className="flex items-center gap-2 text-sm text-slate-400 bg-slate-800/40 p-2 rounded-md border border-slate-700/50">
                  <div className="w-2 h-2 rounded-full bg-green-500" />
                  {table}
                </div>
              ))}
              {(!response?.metadata?.found_in_db || response.metadata.found_in_db.length === 0) && (
                <p className="text-xs text-slate-600 italic">No schema context loaded</p>
              )}
            </div>
          </div>
              ...
              {history.map(item => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-xs text-slate-300 truncate flex-1 group-hover:text-blue-400">{item.question}</p>
                    {item.engine && (
                      <span className={`text-[8px] px-1.5 py-0.5 rounded border ${getEngineColor(item.engine)}`}>
                        {item.engine}
                      </span>
                    )}
                  </div>
                  <span className="text-[10px] text-slate-600 uppercase">{item.timestamp}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="bg-slate-800/30 p-4 rounded-xl text-center">
            <p className="text-xs text-slate-500 uppercase font-bold tracking-tighter">Connected via n8n</p>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#0F1115] relative">
        <header className="h-16 border-b border-slate-800 flex items-center justify-between px-8 bg-[#16191F]/50 backdrop-blur-md">
          <div className="flex items-center gap-4">
             <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-slate-800 rounded-md transition">
               <ChevronRight className={`w-5 h-5 transition-transform ${sidebarOpen ? 'rotate-180' : ''}`} />
             </button>
             <div className="h-4 w-px bg-slate-800" />
             <span className="text-sm font-medium text-slate-400 flex items-center gap-2">
               Workspace / 
               <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded border ${getEngineColor(dbEngine)}`}>
          ...
          <div className="flex items-center gap-3">
            <div className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-tighter uppercase ${isLoading ? 'bg-blue-500/10 text-blue-400 animate-pulse' : 'bg-slate-800 text-slate-500'}`}>
        ...
        <div className="p-8 pb-0 shrink-0">
          <div className={`bg-[#16191F] rounded-2xl border ${dbEngine === 'spark' ? 'border-orange-500/30' : 'border-slate-800'} p-6 shadow-2xl relative overflow-hidden group`}>
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-3 space-y-4">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Target Tables</label>
                <input 
                  type="text"
                  placeholder={dbEngine === 'spark' ? "e.g. catalog.db.table" : "e.g. users, sales"}
                  value={tables}
                  onChange={(e) => setTables(e.target.value)}
                  className={`w-full bg-[#0F1115] border border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 ${dbEngine === 'spark' ? 'focus:ring-orange-500/50' : 'focus:ring-blue-500/50'} transition`}
                />
              </div>
              <div className="lg:col-span-7 space-y-4">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className={`w-full bg-[#0F1115] border border-slate-700 rounded-xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:ring-2 ${dbEngine === 'spark' ? 'focus:ring-orange-500/50' : 'focus:ring-blue-500/50'} transition`}
                  />
                </div>
              </div>
              <div className="lg:col-span-2 flex items-end">
                <button 
                  ...
                  disabled={isLoading}
                  className={`w-full ${dbEngine === 'spark' ? 'bg-orange-600 hover:bg-orange-500 shadow-orange-900/20' : 'bg-blue-600 hover:bg-blue-500 shadow-blue-900/20'} disabled:bg-slate-700 text-white rounded-xl py-3 px-6 font-bold flex items-center justify-center gap-2 transition-all shadow-lg`}
                >
                  {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : dbEngine === 'spark' ? <Zap className="w-4 h-4" /> : <Play className="w-4 h-4 fill-current" />}
        ...
        <div className="flex-1 p-8 overflow-hidden flex flex-col gap-6">
                ...
                <div className="bg-[#16191F] rounded-2xl border border-slate-800 flex flex-col overflow-hidden">
                  <div className="px-5 py-3 border-b border-slate-800 flex justify-between items-center bg-slate-900/20">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                      {response.engine === 'spark' && <Zap className="w-3 h-3 text-orange-400" />}
                      AI Generated {response.engine === 'spark' ? 'Spark' : ''} SQL
                    </span>
                    <button 
                      onClick={() => copyToClipboard(response.query)}
                      className={`p-1.5 rounded transition ${copied ? 'text-green-500 bg-green-500/10' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
                    >
                      {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                  <div className="p-5 font-mono text-sm max-h-[150px] overflow-y-auto">
                    <code className={`${response.engine === 'spark' ? 'text-orange-400' : 'text-blue-400'} block whitespace-pre-wrap`}>
                      {response.query}
                    </code>
                  </div>
                </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-center">
                    {response.engine === 'spark' ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-orange-400">Spark SQL Generated</p>
                          <p className="text-xs text-slate-500">Query ready for your Spark cluster. Execution not performed.</p>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-center gap-3">
                        {response.success ? <CheckCircle2 className="text-green-500" /> : <AlertTriangle className="text-yellow-500" />}
                        <div>
                          <p className="text-sm font-semibold">{response.success ? 'Query Success' : 'Execution Warning'}</p>
                          <p className="text-xs text-slate-500">{response.success ? `Data retrieved from ${dbEngine}` : 'There was an issue executing the SQL.'}</p>
                        </div>
                      </div>
                    )}
                  </div>
                    ...
                    {response.results?.length > 0 ? (
                      <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 z-20 bg-[#1A1E24]">
                          <tr>
                            {getColumns().map(col => (
                              <th key={col} className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase border-b border-slate-800">{col}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/50">
                          {response.results.map((row, idx) => (
                            <tr key={idx} className="hover:bg-blue-500/[0.02] transition-colors">
                              {getColumns().map(col => (
                                <td key={col} className="px-6 py-4 text-sm text-slate-400">
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-slate-600 gap-4">
                        ...
                        <p className="text-sm italic">No data returned for this query</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {response.engine === 'spark' && (
                <div className="flex-1 bg-[#16191F] rounded-2xl border border-orange-500/20 flex flex-col overflow-hidden shadow-2xl">
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <h3 className="text-xl font-bold text-slate-300 mb-2">Query Generated Successfully</h3>
                    <p className="text-sm text-slate-500 max-w-md mb-6">
                      Your Spark SQL query has been generated. Copy the SQL above and execute it in your Spark environment (Databricks, EMR, etc.).
                    </p>
                    <button 
                      onClick={() => copyToClipboard(response.query)}
          ...
          {!response && !isLoading && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-6">
              <div className={`bg-slate-800/30 p-8 rounded-full border ${dbEngine === 'spark' ? 'border-orange-500/20' : 'border-slate-800'}`}>
                {dbEngine === 'spark' ? (
              ...
              <div className="text-center">
                <h3 className="text-xl font-bold text-slate-400">
                  Ready to {dbEngine === 'spark' ? 'Generate' : 'Query'} {dbEngine.toUpperCase()}
                </h3>
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #2D333D; border-radius: 10px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #3D4451; }
      `}</style>
    </div>
  );
};
